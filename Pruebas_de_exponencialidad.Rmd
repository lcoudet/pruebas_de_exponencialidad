...
Un experimento en Florida (EEUU) diseñado para estudiar el efecto dela aplicación de dosis de metimelcurio (un veneno) en el tiempo de vida de los peces.

Los peces fueron sometidos a varias dosis del veneno.

Al nivel de una dosis, se obtuvieron los siguientes \textbf{tiempos de muerte ordenados} (en días):
...
en dónde $\bar{F(x)} = 1 - F(x)$

## Interpretación

$r(x) \delta_x$ es la probabilidad de que un ítem (unidad, perosona, parte) viva a la edad x va a fallar en el intervalo $(x, x + \delta_x)$ con $\delta_x$ pequeño.
Si:
...
$$ H_0: r(x)=\lambda $$
La hipótesis nula de exponencialidad implica suponer una \textbf{tasa constante}, es decir \textbf{\underline{independiente de la edad}}, el cuál es supuesto de la distribución exponencial. A veces se dice que la distribución exponencial \textit{no tiene memoria}, lo cual se debe a lo anterior.
...
-  $r(x)$ es IFR sí $r(x) \leq r(y) \, \forall \, x<y$.
-  $r(x)$ es DFR sí $r(x) \geq r(y) \, \forall \, x<y$

# Prueba y estadistico de prueba

Sea $X_1, \cdots, X_n$ una muestra de $X$ y sean $X_{(1)}, \cdots, X_{(n)}$ los estadísticos de orden, con $X_{0}=0$ se consideran los espacion normalizados $D_1, \cdots, D_n$:
...
$H_1: F$ es IFR ó DFR (y por lo tanto no exponencial)
...
-  Alternativa IFR ó DFR: $|\epsilon^{*}| \geq Z_{\frac{\alpha}{2}}$.


# Methylmercury poisining: One sided test contra alternativa IFR

Se quiere testear la hipótesis nula de exponencialidad (tasa $r(x)$ constante) contra la alternativa de tasa creciente, debido a que se sospecha que cuantos más días pasan más crece la probabilidad de muerte.
...
El estadístico de prueba $\epsilon$ toma valor 7.740741. Buscamos en la tabla para $n=10$ y encontramos $P<.005$ por lo cual existe evidencia empírica en contra d ela hipótesis nula de exponencialidad y a favor de la alternativa IFR.
...
$en$ toma valor 3.742085 por lo que $P < .002$ y por lo tanto la aproximación para muestras grandes confirma la evidencia contra $H_o$ y a favor de \textit{\textbf{age deteroration}}.


# Comentarios: Total time on test transforms

$n$ ítems independientes se ponen a prueba para estudiar su supervivencia. Sean $X_1, \cdots, X_n$ sus tiempos de vida observados y sean $X_{(1)}, \cdots, X_{(n)}$ sus valores ordenados (los estadísticos de orden), entonces definimos el tiempo total en prueba al momento $X_{(i)}$:

$$ n X_{(1)} + (n-1)(X_{(2)} - X_{(1)}) + \cdots (n- i + 1)(X_{(i)} - X_{(i-1)}) = $$
$$ \sum_{u = 1}^i (n - u + 1)(X_{(u)} - X_{(u-1)}) = \sum_{u=1}^i D_U = S_i $$
Se transforman los estadísticos de orden en $T_1, \cdots, T_n$ de la sieguiente manera:
...
-  Bajo $H_0$ cierta, $T_1, \cdots, T_n$ tienen la misma distribución que los estadísticos de orden de una muestra de tamaño $(n-1)$ de una distribución uniforme $(0,1)$.
...
La clase IFRA considera el caso por ejemplo en el que $r(x)$ fluctúa por ejemplo debido a variaiones estacionales. En aplicaciones médicas, por ejemplo, una tasa $r(x)$ inicialmente creciente puede decrecer debido a una intervención médica.
