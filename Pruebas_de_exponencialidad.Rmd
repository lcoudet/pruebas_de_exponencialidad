...
Los cual bajo la hipótesis nula de exoinencialidad corresponde a una suma de VA $U(0,1)$ iid (ver Epstein, 1960) y por lo tanto: $E_0(\varepsilon) = \frac{n-1}{2}$ y $V_0(\varepsilon) = \frac{n-1}{12}$.
...
El estadístico de prueba $\epsilon$ toma el valor `r round(epsilon,4)`. Buscamos en la tabla para $n=10$ y encontramos $P<0.005$ por lo cual existe evidencia empírica en contra de la hipótesis nula de exponencialidad y a favor de la alternativa IFR.
...
$en$ toma el valor `r round(en,4)` por lo que $P < 0.002$ y por lo tanto la aproximación para muestras grandes confirma la evidencia contra $H_0$ y a favor de \textit{\textbf{age deteroration}}.
...
$$H0: P(X \geq x + y | X \geq x) = P(X \geq y) \,\,\,\, \forall x;y \geq 0$$

la cual podemos reescribir utilizando las funciones de vida como:

$$H0: \frac{ \bar{F}(x+y) }{ \bar{F}(x) } = \bar{F}(y) \,\,\,\, \forall x;y \geq 0$$

### Relación con las clases NBU y NWU

- Si $\bar{F}(x+y) \leq \bar{F}(x) \bar{F}(y) \,\,\,\, \forall ;y \geq 0$, entonces la clase es NBU.  

- Si $\bar{F}(x+y) \geq \bar{F}(x) \bar{F}(y) \,\,\,\, \forall ;y \geq 0$, entonces la clase es NWU.  

## Estadístico de prueba

- Para construir el estadístico deprueba primero debemos ordenar la muestra.  
...
- Prueba a dos colas contra alternativa de NBU o NWB: rechazar H0 si $T \leq t_{1;\alpha_1}$ o si $T \geq t_{1;\alpha_2}$ para un nivel de significación $\alpha_1 + \alpha_2 = \alpha$.


## Aproximaciones para muestras grandes

- Se define el estadístico: 
$$T^* = \frac{ T - E_{H0} (T) }{ \sqrt{V_{H0}(T)} } \overset{d}{\rightarrow} N(0; \,1) $$

\vspace{0.5cm}

- Prueba a una cola contra alternativa de NBU: rechazar H0 si $T^* \leq -z_{\alpha}$ para un nivel de significación $\alpha$.  

\vspace{0.15cm}

- Prueba a una cola contra alternativa de NWU: rechazar H0 si $T^* \geq z_{\alpha}$ para un nivel de significación $\alpha$.

\vspace{0.15cm}

- Prueba a dos colas contra alternativa de NBU o NWB: rechazar H0 si $T^* \leq -z_{\alpha_1}$ o si $T^* \geq z_{\alpha_2}$ para un nivel de significación $\alpha_1 + \alpha_2 = \alpha$.
